...
mod tests {
    ...
    fn test_has_staged_changes() {
        ...
        assert!(!has_staged_changes(" M src/main.rs\n"));
        assert!(!has_staged_changes("?? src/new.rs\n"));
        assert!(!has_staged_changes(""));
        assert!(has_staged_changes("MM src/main.rs\n"));
        assert!(has_staged_changes("A  src/new.rs\n"));
        assert!(has_staged_changes("D  src/old.rs\n"));
    }

    #[test]
    fn test_has_unstaged_changes() {
        assert!(has_unstaged_changes(" M src/main.rs\n"));
        assert!(!has_unstaged_changes("M  src/main.rs\n"));
        assert!(has_unstaged_changes("?? src/new.rs\n"));
        assert!(!has_unstaged_changes(""));
        assert!(has_unstaged_changes("MM src/main.rs\n"));
        assert!(has_unstaged_changes(" D src/old.rs\n"));
    }

    #[test]
    fn test_get_change_status_from_porcelain() {
        let status = get_change_status_from_porcelain("M  src/staged.rs\n M src/unstaged.rs\n");
